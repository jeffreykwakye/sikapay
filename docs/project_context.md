...
| **Flexible Payroll Workflow** | Refactoring the payroll process to support a multi-stage workflow (Open/Draft -> Closed), allowing for re-runs and corrections before finalizing. | In Progress | This will replace the rigid, linear payroll run with a more flexible, state-driven process. Payslips are viewable for open periods, but draft statutory reports are not yet viewable. |
...
| **2025-12-13** | **Flexible Payroll Workflow** | Began refactoring the core payroll module to transition from a rigid, linear process to a flexible, state-driven one (Open -> Closed). This will allow for payroll re-runs, corrections, and manual closing of periods before payslip notifications are sent. Payslips are viewable for open periods, but draft statutory reports are not yet viewable. | In Progress |
...
| **2025-12-13** | **Professional Statutory Reports & Cover Letters, CSV Generation, and Tiered Availability** | Redesigned PAYE, SSNIT, and Bank Advice PDF reports for a professional and consistent look. Implemented tenant-configurable cover letters with dynamic addressing and amounts in words. Added CSV report generation for PAYE, SSNIT, and Bank Advice. Implemented tiered availability for report formats (PDF for Standard, PDF/CSV for Professional, PDF/CSV/Excel for Enterprise) based on subscription plans. This involved creating a database migration, updating the company profile UI, and refactoring all PDF and new CSV generator classes. | **Complete** |
...
| **2025-12-08** | **Payroll Element Fix** | Fixed issue where adding/updating payroll elements failed on live server due to `calculation_base` truncation and boolean handling. | **Complete** |
| **2025-12-08** | **Subscription Management Page** | Created a dedicated "How to Renew/Manage Subscription" page (`/subscription/how-to-pay`) with manual payment instructions (bank and mobile money details), updated support contact information, and abstracted internal roles. Linked from the main subscription page. | **Complete** |
| **2025-12-08** | **Payslip PDF Design (Initial)** | Implemented a professional redesign for the Payslip PDF (`PayslipPdfGenerator.php`). This included a new header layout with company logo, payslip title and period. Employee and company details were moved below the header in a two-column format. Net Pay section was updated (label, alignment). | **In Progress** | Design iteration is ongoing. |
| **2025-12-08** | **Payslip PDF Design (Fix: Pay Date N/A)** | Resolved the `pay_date` showing "N/A" by making `payment_date` a required field in payroll period creation. | **Complete** |
| **2025-12-08** | **Payslip PDF Design (Fix: Headers)** | Redesigned Earnings and Deductions tables with clearer headers and sub-headers ("Item", "Amount (GHS)") spanning their respective sections. | **In Progress** | Further refinement on borders. |
| **2025-12-08** | **Payslip PDF Design (Color Update)** | Changed the primary header color to Teal for a fresh, professional look across all reports, while retaining a darker blue for the Net Pay summary. | **In Progress** |
| **2025-12-08** | **Payslip PDF Design (Fix: Border Gaps)** | Refactored border drawing logic in Earnings/Deductions tables to ensure seamless vertical and horizontal lines, resolving visible gaps. | **In Progress** | Further refinement on borders. |
| **2025-12-08** | **Payslip Test Sample Setup** | Created a test route (`/test/payslip-sample`) and controller method (`TestController::payslipSample`) to generate a sample payslip PDF with hardcoded data for easy design iteration. Removed permission requirement for testing. | **Complete** |
